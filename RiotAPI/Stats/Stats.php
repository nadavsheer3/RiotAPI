...
class Stats extends RiotApiController{
	...
	static public function rankedStatsById($id, $server, $season = 'SEASON2016', $api_key){
		
		$url ='https://'.$server.'.api.pvp.net/api/lol/'.$server.'/v1.3/stats/by-summoner/'.$id.'/ranked?season='.$season.'&api_key='.$api_key;
	...
	//	If specified, stats for the given season are returned. Otherwise, stats for the current season are returned
	static public function summaryStatsById($id, $server, $season = 'SEASON2016', $api_key){
		
		$url ='https://'.$server.'.api.pvp.net/api/lol/'.$server.'/v1.3/stats/by-summoner/'.$id.'/summary?season='.$season.'&api_key='.$api_key;
		$request = $this->apiFetch($url);
